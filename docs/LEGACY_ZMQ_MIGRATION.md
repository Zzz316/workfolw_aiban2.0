...
|------|------|
| **当前（阶段 0）** | 仅标记，不删除任何代码 |
| **阶段一验收后** | 删除 `tests/test_*zmq*`、`tests/node_red_inbox_probe.py`、`data/frame_bridge/*.db` |
...
回退窗口：新阶段一验收前完整保留；阶段一验收后可回退到 `legacy-zmq-baseline` 标签。
