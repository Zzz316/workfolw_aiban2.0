...
> 当前状态：单条线性顺序逻辑已跑通，进入“运行时稳定化、结果分层、group/scene 骨架”阶段<br>
...
- 当前 Node.js 全量测试为 91 项，2026-07-22 本地执行结果为 91/91 通过。
...
按“代码、自动化测试、真实环境验证、文档和验收”综合计算，当前整体工程成熟度估算为 **35%（±5%）**。其中单 Runtime + 单线性顺序场景约 70%，多 group/multi-scene 平台能力约 10%，1.0 全业务功能对等约 20%，生产发布成熟度约 15%。百分比不是代码行完成率，详细依据见配套任务书的模块进度表。

尚未完成的关键项：

- 启停按钮的显示状态与 Pipeline 实际状态尚未完全一致。
- 管理接口返回成功不代表 SDK 已经到达 `READY` 或 `STOPPED`。
- 消息输入控制与编辑器按钮使用了不同的控制路径。
- Python `restart` 命令存在停止后退出事件未清理的问题，需要修复并补强测试。
...
- README、真实 SDK 测试说明和 1.0 功能对等矩阵存在状态滞后。
...
|---|---|---:|---|---|---:|---:|
| M0 基线冻结 | T00 | 2 | 07-23～07-24 | 07-24 | 40% | 36% |
| M1 Runtime 稳定化 | T01～T04 | 11 | 07-27～08-10 | 08-10 | 70% | 43% |
...
剩余工作基准合计 104 人日，功能和发布任务目标完成日为 2026-12-15；另预留约 15% 风险缓冲，管理目标完成日为 2026-12-31。详细到任务的日期、进度和依据见 [`WORKFLOW_V2_DEVELOPMENT_TASK_SPEC.md`](WORKFLOW_V2_DEVELOPMENT_TASK_SPEC.md)。
...
1. 执行任务 `T00`，冻结当前测试和文档基线。
2. 执行 `T01`～`T04`，完成 Runtime 生命周期稳定化。
3. 执行 `T05`～`T08`，冻结 outcome/result 契约并分层组件。
