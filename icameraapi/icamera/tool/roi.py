...
pts = []  # 用于存放点
point = ''
config = configparser.ConfigParser()  # 配置文档实例化
...
def draw_roi(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MBUTTONDOWN:  # 中键绘制轮廓
        ...
        show_image = cv2.addWeighted(src1=img, alpha=0.8, src2=mask3, beta=0.8, gamma=0)
...
while True:
    ...
    if key == ord("s"):
        for i in range(len(pts)):
            p = pts[i]
            px = round(p[0] * 1920 / img.shape[1])
            py = round(p[1] * 1080 / img.shape[0])
            ppstr = str(px)+','+str(py)
            point = point + ppstr + ';'
        config.set('ROI', 'points_1', str(point))
